...
function photoPreviewBlock(){
  ...
  return `<div class="photo-review-block pop">
    ...
    <img src="${window.pendingPhotoDataUrl}" alt="Foto del pedido" class="photo-review-img">
  </div>`;
}
...
function renderInterpretSheet(){
  ...
  const rows=items.filter(it=>!it.removed).map(it=>{
    ...
    return `<div class="rev-row ${it.highlight!=='none'&&!it.resolved?'warn':''}" data-ri="${i}">
      ...
      <div class="rn">
        ${!p?`<select onchange="fixMatch(${i},this.value)"><option value="">— Elige producto —</option>${opts}</select>`:`<b>${p.name}</b>`}
      ...
      ${isAdmin&&it.uCliente?`<span class="chip exc" style="font-size:10px">${esc(unitLabel)}</span>`:
        `<select style="border:2px solid var(--line);border-radius:10px;padding:6px;font-weight:700;${it.uCliente?'display:none':''}" onchange="pendingItems[${i}].unidad=this.value;pendingItems[${i}].unidadSistema=this.value;draftInterpret()">
          ${uopts.map(u=>`<option value="${u.id}" ${(it.unidadSistema||it.unidad||'kilo')===u.id?'selected':''}>${u.short}</option>`).join('')}
        </select>`}
      <input class="qty-in" style="width:52px" inputmode="decimal" value="${it.cantidad||1}" oninput="pendingItems[${i}].cantidad=+this.value||1;draftInterpret()">
